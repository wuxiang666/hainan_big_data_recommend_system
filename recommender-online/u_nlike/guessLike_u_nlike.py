...
def loggers():
    ...
    logger = logging.getLogger('u_nlike')
...
def main():
    ...
    for item in ps.listen():
        ...
        try:
            ...
            logger.error('enable to get data from hainan_bigdata_Mysql,the uid is: %s,please check'% uid)
            cursor, db = mysql_con(logger)
            try:
                cursor.execute(sql)
                db.commit()
                ...
                logger.error('the mysql connect problem is still not fixed.please check')
